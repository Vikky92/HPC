...
typedef int boolean;
//Representiation of edges in the graph with both the end points 
typedef struct
{
	int u;
	int v;
...
//Representation of the vertices in the graph with the node id and a boolean visited value for tracking the nodes
//Represents a Vertex
typedef struct 
{
	int title;
...
//Used to find the weight of the path from u to v
...
*/	  
	...
	for(i = 0; i < V; i++){
		...
		int numBlocks, numThreads;
			...
			Find_Vertex<<<numBlocks, numThreads>>>(d_V, d_E, d_W, d_L, d_C);
