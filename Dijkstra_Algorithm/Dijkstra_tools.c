...
This has been used from along with the header Dijkstra_tools.h from a previous implementation of the serial Dijkstra's algorithm for another project 

These files contain the definitions for allocating the memory and definitions of the graph structures computed in the Serial and Parallel implementation of the algorithm 
...
//This was an initial draft for a given number of nodes. Here a manual assignment of the nodes was done for testing purposes 
void GraphTestGenerate(graph* G)
{
	char* p;

	GraphEmptyGenerate(6,G); // This is used to malloc for a emptyTestGraph with 6 nodes 
	p = &G->node[0][0];
...
//allocates memory for the G->D and the G->visited and G-node to store the graph structure
void GraphEmptyGenerate(long N, graph *G)
{
	int i;
...
//This is the final implementation of the generation of graph used to assign the weights and the links in the graph 
// we have used the rand() function whos initial value is obtained from the commandline to assign weights and the connections have been made with a 50:50 ratio
//NO_CONN between two links is used to indicate absence of a connection between the respective nodes

void GraphGenerate(long N, int randInit, graph *G, char debug)
{
	long linkCnt;
	long i,j;
	long t;
	...
	//To nodes that have no incoming edge, add one randomly
	for(i=0;i < G->N; i++){
...
long par_NextNode(graph* G, long nStart, long nEnd)
{
	long i;
	long minD;
	long nextNode;
